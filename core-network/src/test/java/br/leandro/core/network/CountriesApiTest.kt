package br.leandro.core.network

import br.leandro.core.network.api.TheSportsDbApi
import br.leandro.core.network.util.NetworkProvider.createRetrofitServer
import br.leandro.core.network.util.NetworkProvider.getJsonString
import junit.framework.TestCase.assertEquals
import kotlinx.coroutines.test.runTest
import okhttp3.mockwebserver.MockResponse
import okhttp3.mockwebserver.MockWebServer
import org.junit.Before
import org.junit.Test
import retrofit2.Retrofit
import retrofit2.converter.gson.GsonConverterFactory

class CountriesApiTest {
    private lateinit var server : MockWebServer
    private lateinit var api : TheSportsDbApi

    @Before
    fun setup() {
        server = MockWebServer()
        api = createRetrofitServer(server)
    }

    @Test
    fun `getCountries should return countries list when response is successful`() = runTest {
        val json = getJsonString("country_response.json")
        server.enqueue(MockResponse().setResponseCode(200).setBody(json))
        val response = api.getCountries()
        assertEquals(3, response.countries.size)
        val country = response.countries[0]
        assertEquals("Andorra", country.name_en)
        val request = server.takeRequest()
        assertEquals("/all_countries.php", request.path)


    }

    @Test(expected = Exception::class)
    fun `getCountries should throw exception when response is not successful`() = runTest {
        server.enqueue(MockResponse().setResponseCode(404))
        api.getCountries()


    }

}